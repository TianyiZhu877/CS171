...
void update_pendulum()
{
     ...
     * 
     */
    double next_x, next_y, next_px, next_py, r;
    double x = static_cast<double>(m1.x);
    double y = static_cast<double>(m1.y);
    double px = static_cast<double>(m1.px);
    double py = static_cast<double>(m1.py);
    double k = static_cast<double>(m1.k);
    double l = static_cast<double>(m1.rl);
    double m = static_cast<double>(m1.m);

    r = sqrt(x*x+y*y);

    next_x = (px - k*(r - l)/r*x*dt)*dt/m + x;
    next_y = (py - (k*(r - l)/r*y + m*g)*dt)*dt/m + y;
    next_px = m*(next_x-x)/dt;
    next_py = m*(next_y-y)/dt;


    m1.x = static_cast<float>(next_x);
    m1.y = static_cast<float>(next_y);
    m1.px = static_cast<float>(next_px);
    m1.py = static_cast<float>(next_py);
