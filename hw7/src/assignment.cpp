...
static double S(double x, double y, double z, double e, double n) {
    return -1 + pow(z*z, 1/n) + pow(pow(x*x, 1/e) + pow(y*y, 1/e), e/n);
}

static Vector3d apply_inverse_transform(const vector<unique_ptr<Transformation>>& transforms, const Vector3d &point) {
    Matrix4d transform_matrix = Matrix4d::Identity();
    for (const auto &transform : transforms) {
        transform_matrix = transform->GetMatrix() * transform_matrix;
    }
    Vector4d transformed_point = transform_matrix.inverse() * Vector4d(point.x(), point.y(), point.z(), 1);

    return Vector3d(transformed_point.x()/transformed_point.w(), transformed_point.y()/transformed_point.w(), transformed_point.z()/transformed_point.w());
}
...
bool Superquadric::IOTest(const Vector3d &point) {
     ...
     */
    Vector3d transformed_point = apply_inverse_transform(transforms, point);
    return S(transformed_point.x(), transformed_point.y(), transformed_point.z(), exp0, exp1) <= 0;
}
...
bool Assembly::IOTest(const Vector3d &point) {
    ...
    for (auto &child : children) {
        if (child->IOTest(transformed_point)) {
            return true;
        }
    }
    return false;
